...
# count the number of times each word occurs in train data
...
# removes the word that only occurrs once in train data in input file
...
def bigram_log_perplexity(sentence, bigram, unique_words, is_smoothing):
  ...
  for i in range(len(input)-1):
    ...
    if two_token in bigram:
      ...
      # token not in training bigram dictionary
      if is_smoothing:
  ...
  ### toDo: check which parameter have a prob of zero
...
def perplexity_bigram(input_file, bigram, unique_words, is_smoothing):
  ...
  for i in range(len(input)-1):
    two_token = input[i] + input[i+1]
    # probability[two_token] = bigram[input[i]][input[i+1]]
    if two_token in bigram:
